  ...
  function renderQuestions() {
    ...
    list.querySelectorAll('.question-select').forEach(select => {
      select.addEventListener('change', () => {
        ...
        if (qid === 'q11' && value === 'Over 60 Knots / 110 kph') {
          openDeclineModal();
          return;
        }
  ...
  list.innerHTML = vesselFields.map(f => {
    ...
    return `
      <div class="field-row" data-field-row="${f.id}">
        <p class="field-label">
          ${f.label} <span class="required">*</span>
        </p>
...
document.getElementById('backBtn2').addEventListener('click', () => goToSection(1));
document.getElementById('continueStep2Btn').addEventListener('click', () => {

  if (quoteState.importantInfo.q1 === 'false') {
    openDeclineModal();
    return;
  }
